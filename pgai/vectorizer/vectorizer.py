...
# Add httpx for making HTTP requests
import httpx
...
from .embedders import LiteLLM, Ollama, OpenAI, VoyageAI, FastAPIHFLocal
...
class Config(BaseModel):
    ...
    embedding: OpenAI | Ollama | VoyageAI | LiteLLM | FastAPIHFLocal = Field(
    ...
    async def run(self) -> int:
        ...
        async with await psycopg.AsyncConnection.connect(
            ...
            try:
                ...
                # The setup for the embedder is still called here
    ...
    async def _generate_embeddings(
        ...
        try:
            ...
            if self.vectorizer.config.embedding.implementation == "fastapi_hf_embed":
                # Directly call the FastAPI server from here
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    # The FastAPI server returns {"embeddings": [[...], [...]], "dimensions": 384}
                    # We need the list of embedding vectors
                    ...
                    for record_base, embedding_list in zip(
                        ...
                        # `embedding_list` is already a list of floats (e.g., [0.1, 0.2, ...])
                        # Convert it to a numpy array for pgvector compatibility
                        records_with_embeddings.append(record_base + [np.array(embedding_list)])
                    yield records_with_embeddings, []
            else:
                # This is the existing logic for other embedders (OpenAI, Ollama, etc.)
